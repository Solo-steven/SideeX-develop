...
const Pull = (props)=>{
    ...
    const handle_Submit = (e)=>{
        e.preventDefault();
        if(UIState.currentRoot.type === "tree")
            return ;
    ...
    return(
        <form onSubmit={(e)=>{handle_Submit(e)}}>
            ...
            <div className="inputBlock">
                ...
                <select value={UIState.currentRoot.path} 
                        onChange={(e)=>{dispatch(UIActionGenerator.change_Current_Root(e.target.value))}}>
                    <option value="current root">{UIState.currentRoot.path}</option> 
                    {
                        !UIState.currentRoot.path ? 
